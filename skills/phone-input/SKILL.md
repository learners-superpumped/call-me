...
- `to` (string, 선택): 전화할 번호 (예: 01012345678). `CALLME_UNSAFE_NO_NUMBER_RESTRICTION=true`일 때만 사용 가능. 생략하면 기본 사용자 번호로 발신.
...
기본적으로 아웃바운드는 등록된 사용자 번호로만, 인바운드는 화이트리스트 번호만 허용됩니다.

`CALLME_UNSAFE_NO_NUMBER_RESTRICTION=true` 설정 시:
- **아웃바운드**: `initiate_call`의 `to` 파라미터로 임의 번호 호출 가능
- **인바운드**: 화이트리스트 체크 없이 모든 번호에서 수신 허용

> ⚠️ **경고**: 이 설정으로 인해 발생하는 요금, 악용, 규제 위반 등 모든 문제는 운영자의 책임입니다.
